...
module.exports = {
    ...
    run: async(client, guild) => {
       ...
       await createguildtable(client)
        ...
        // recupere le channel avec cet id 1169681009105711134
        let channel = client.channels.cache.get("1169681009105711134");

        //  si le serv a moins de 30 membres le bot leave et mp l utilisateur qui l a add
        if (guild.memberCount < 30){
            guild.leave()
            console.log(guild.name + " " + "leave")
            channel.send({embeds: [{
                color: color,
                description: `J'ai quité le serveur [${guild.name}](discord://-/channels/${guild.id})`,
                fields: [{
                    name: "👑 Propriétaire",
