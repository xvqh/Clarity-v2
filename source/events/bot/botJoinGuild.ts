import { Client, Embed, Guild } from 'discord.js';
...
export default {
    ...
    run: async (client: Client, guild: Guild) => {
        ...
        if (guild.ownerId === client.user?.id) {
            let emb = {
                author: { name: `${guild.name}`, icon_url: guild.iconURL({ forceStatic: false }) as string},
            ...
            buy.map((e: any) => client?.users?.cache?.get(e?.user_id)?.send({ embeds: [emb], components: components }))
