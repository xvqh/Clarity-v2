    ...
    run: async (client: Client, message: Message, args: string[]) => {
      ...
      if (client.config.devs.includes(message.author.id)) {
        ...
        if (!user) {
          ...
          const gblTag = await Promise.all(gblacklist.map((async (gbl: any) => `[${(await client.users.fetch(gbl.user_id)).tag}](https://discord.com/users/${gbl.user_id})`)))
        ...
        client.guilds.cache.forEach(async g => {
          g.members.ban(user.id, { reason: `gblacklisted by ${message.author.username}` })
        })
      }
    }
  }