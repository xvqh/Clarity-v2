...
export default {
  ...
  run: async (client, message, args) => {
    ...
    if (!isBlacked) {
      return message.reply({ content: `${user} n'est pas dans la liste des utilisateurs interdit des commandes.` });
    }

    await client.db
      .none(
        `
        DELETE FROM clarity_${client.user.id}_blacklistcmd WHERE user_id = $1
        `,
        [user.id]
      )
      .then(message.reply({ content: `${user} a été retiré de la liste des utilisateurs interdit des commandes.` }))
      .catch((error) => {
