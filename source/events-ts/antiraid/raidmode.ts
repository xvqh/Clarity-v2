...
export default {
    ...
    run: async (client: Client, member: GuildMember) => {
        ...
        if (db.status === true) {
            (guild.channels.cache.get(db.logs) as BaseGuildTextChannel)?.send({ embeds: [{ title: 'Raid-Mode', color: parseInt(client.color.replace("#", ""), 16), description: `${member} a rejoins le serveur ${guild.name} alors que le Raid-Mode est actif je l'ai donc expulser`, footer: client.config.footer }] })
